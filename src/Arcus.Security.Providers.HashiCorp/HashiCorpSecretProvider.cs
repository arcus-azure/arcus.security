...
namespace Arcus.Security.Providers.HashiCorp
{
    ...
    {
        private const string DependencyName = "HashiCorp Vault";
            ...
            ILogger<HashiCorpSecretProvider> logger)
        {
            ...
            _logger = logger ?? NullLogger<HashiCorpSecretProvider>.Instance;
        }
        ...
        public async Task<string> GetRawSecretAsync(string secretName)
        {
            Guard.NotNullOrWhitespace(secretName, nameof(secretName), 
                $"Requires a non-blank secret name to look up the secret in the HashiCorp Vault {_options.KeyValueVersion} KeyValue secret engine");
        ...
        {
            switch (_options.KeyValueVersion)
            {
                case VaultKeyValueSecretEngineVersion.V1:
                    Secret<Dictionary<string, object>> secretV1 = 
                        await _vaultClient.V1.Secrets.KeyValue.V1.ReadSecretAsync(secretPath, mountPoint: _options.KeyValueMountPoint);
                    return new SecretData { Data = secretV1.Data };
                
                case VaultKeyValueSecretEngineVersion.V2:
                    Secret<SecretData> secretV2 = 
                        await _vaultClient.V1.Secrets.KeyValue.V2.ReadSecretAsync(secretPath, mountPoint: _options.KeyValueMountPoint);
                    return secretV2.Data;
                
                default:
                    throw new ArgumentOutOfRangeException(nameof(_options.KeyValueVersion), _options.KeyValueVersion, "Unknown HashiCorp Vault KeyValue secret engine version");
            }
        }
    }
}
